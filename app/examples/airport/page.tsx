...
export const metadata = {
  title: "Airport — Trinisette example",
  description:
...
export default async function AirportNowPage({ searchParams }: PageProps<"/examples/airport">) {
    ...
    <>
      <header className="sticky top-0 z-20 border-b border-line bg-bg/85 backdrop-blur">
        <div className="mx-auto flex max-w-[1320px] items-center gap-6 px-6 py-3.5 md:px-8">
          ...
          <Link href="/cases/airport" className="ml-auto text-[12.5px] text-fg-3 transition-colors hover:text-fg">
            Why this shape ↗
          </Link>
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-[1320px] px-6 py-8 md:px-8 md:py-10">
        <div className="mb-4 flex flex-wrap items-baseline justify-between gap-x-8 gap-y-2">
          <div className="flex items-baseline gap-4">
            <h1 className="text-h2 text-fg">Airport</h1>
            <span className="text-[12.5px] text-fg-3">
              A real domain with real coverage gaps, running on the substrate. Poke at it.
            </span>
          </div>
        ...
        <div className="mb-4 flex flex-wrap gap-x-6 gap-y-1 border-y border-line py-2 text-[11.5px] text-fg-3">
          ...
          <span><span className="text-fg-2">fork</span> is O(1) and shares the parent&rsquo;s root</span>
          <span><span className="text-fg-2">publish</span> is refused outside primary by the runtime, not the UI</span>
          <span><span className="text-fg-2">any posted number</span> traces back to a response hash</span>
          <span><span className="text-fg-2">16 airports</span> have no queue feed — graft answers that</span>
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-baseline gap-x-6 gap-y-2 border-t border-line pt-4 text-[11.5px] text-fg-3">
          <span>
            Wait figures are <span className="text-fg-2">modelled</span>; the coverage map, tiers and
            upstream URLs are real, from the project&rsquo;s own source survey.
          </span>
          <span>
            One live upstream:{" "}
            <a href="https://nasstatus.faa.gov/api/airport-status-information" target="_blank" rel="noreferrer"
              className="text-fg-2 underline decoration-line underline-offset-2 hover:text-accent">FAA NAS Status</a>.
          </span>
          <span>
            Product shape from{" "}
            <a href="https://github.com/mylee04/airport-now" target="_blank" rel="noreferrer"
              className="text-fg-2 underline decoration-line underline-offset-2 hover:text-accent">mylee04/airport-now</a>
            {" "}— domain only, no code (that repo states no licence).
          </span>
          <span className="text-fg-4">Worlds live in memory, so a fork link resolves only in the session that forked it.</span>
        </div>
      </main>
    </>
  );
