...
/**
 * Berth plan — a sketch domain.
 *
 * There is no bespoke product surface here, on purpose. The console below is generic:
 * it renders itself from the declaration in `lib/tri/domains/port.ts` and contains no
 * port code at all. That an ontology written in an afternoon gets forking, addressing,
 * effect suppression and lineage for free is the thing this page is evidence for.
 */
export const metadata = {
  title: "Berth plan — Trinisette example",
  description: DOMAIN_META.port.blurb,
};

export default async function PortPage({ searchParams }: PageProps<"/examples/port">) {
      ...
      <header className="sticky top-0 z-20 border-b border-line bg-bg/85 backdrop-blur">
        <div className="mx-auto flex max-w-[1320px] items-center gap-6 px-6 py-3.5 md:px-8">
          ...
          <nav className="flex items-baseline gap-2 font-mono text-[12px]">
            ...
            <span className="text-fg">port</span>
          </nav>
      ...
      <main className="mx-auto w-full max-w-[1320px] px-6 py-8 md:px-8 md:py-10">
        <div className="mb-4 flex flex-wrap items-baseline justify-between gap-x-8 gap-y-2">
          <div className="flex items-baseline gap-4">
            <h1 className="text-h2 text-fg">Berth plan</h1>
            <span className="max-w-[62ch] text-[12.5px] text-fg-3">{DOMAIN_META.port.blurb}</span>
          </div>
        </div>
        ...
        <div className="mb-4 flex flex-wrap gap-x-6 gap-y-1 border-y border-line py-2 text-[11.5px] text-fg-3">
          ...
          <span><span className="text-fg-2">{DOMAIN_META.port.irreversible.split(" — ")[0]}</span> is refused outside primary</span>
          <span><span className="text-fg-2">graft</span> answers: {DOMAIN_META.port.gap}</span>
        </div>

        <DomainConsole domain="port" initialWorld={w} initialSeq={t} />
