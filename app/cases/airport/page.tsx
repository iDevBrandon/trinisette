...
function input(): ScenarioInput {
  ...
  return {
    airport: "STL", terminal: "Main Terminal", boardingAt: "08:35",
    ...
    curve: [
      ...
      { minutesFromNow: 34, waitMin: 14, prov: { source: "official", detail: "airport-published estimate", ageMinutes: 4 } },
      { minutesFromNow: 79, waitMin: 23, prov: { source: "community", detail: "3 traveller reports", ageMinutes: 18 } },
      // Nothing at STL knows the shape of the morning bank, so it is borrowed from DFW.
      { minutesFromNow: 124, waitMin: 46, prov: { source: "grafted", detail: "pre-bank spike", ageMinutes: 6, originAirport: "DFW" } },
      { minutesFromNow: 164, waitMin: 58, prov: { source: "grafted", detail: "pre-bank spike", ageMinutes: 6, originAirport: "DFW" } },
    ],
  };
}

const MAPPING = [
  { of: "mare" as const, name: "Mare", is: "Forked worlds — a different assumption",
  ...
  { of: "vongola" as const, name: "Vongola", is: "The chain this world came through",
  ...
  { of: "arcobaleno" as const, name: "Arcobaleno", is: "One world at one instant",
    ...
    <>
      ...
      <div className="mx-auto max-w-[1180px] px-6 pt-14 pb-12 md:px-10 md:pt-20">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-4">
          <Link href="/examples/airport" className="btn btn-primary">
            Try it now →
          </Link>
          <span className="font-mono text-[12px] text-fg-3">/examples/airport</span>
        </div>
      ...
      <Section index="01" title="The mapping"
        ...
        <Panel className="mt-px border-t-0 p-6">
          ...
          <p className="mt-3 max-w-[80ch] text-b1 text-fg-2">
            ...
            <br /><br />
            A neighbouring airport fails that test. STL and DFW are two objects in the{" "}
            ...
            transfers; a live reading (&ldquo;DFW is 45 minutes right now&rdquo;) must not.
            <br /><br />
            ...
            the same timeline.
            <br /><br />
            And the mirror of it on the vertical axis: <em className="not-italic text-fg">a past
            reading is not automatically Vongola</em>. Yesterday&rsquo;s curve, sitting in a table
            inside the current snapshot, is data — it has no coordinate and you cannot fork from it.
            Vongola is the chain of states this world actually passed through. The test is the same
            one in both directions: if you cannot address it and branch from it, it is not an axis.
          </p>
        </Panel>
      </Section>
      ...
      <Section index="03" title="Use it"
        ...
        >
          <div className="flex flex-wrap items-center gap-x-8 gap-y-4 px-6 py-7">
            <div>
              ...
              <div className="mt-2 font-mono text-[22px] tracking-[0.1em] text-fg">/examples/airport</div>
            </div>
            <p className="max-w-[46ch] text-b1 text-fg-2">
              Pick a departure airport, walk the clock into the bank, fork, and try to publish an
              alert. The runtime refuses it outside primary and tells you why — that refusal is the
              product, not the object list.
            </p>
        ...
        <div className="mt-6 grid gap-px border border-line bg-line md:grid-cols-3">
          {[
            ["1", "Move the world", "Advance the clock into the departure bank, or open a lane. The posted wait recomputes and the primary value shows struck through."],
            ["2", "Fix the coverage gap", "Pick ORD, SFO, SEA or SJC — real airports with no queue feed. Graft a bank shape from a covered peer and watch every figure downstream get stamped second-hand at 60%."],
            ["3", "Try to publish", "PublishAlert is irreversible. Fork, then press it: the runtime suppresses it — state updates, nothing is emitted."],
          ].map(([n, t, d]) => (
