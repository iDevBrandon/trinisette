...
const INVARIANTS = [
  ...
  ["I5", "Every object carries provenance to an origin", "No anonymous state; contamination is traceable"],
  ["I6", "Irreversible effects only from the primary world", "Forking the agent does not fork the outside world"],
  ["I7", "The ontology mutates only via a declared Action", "An agent cannot write what nobody typed in advance"],
  ["I8", "Every cross-world transfer is a lateral edge", "Provenance audits see imports, not just history"],
];

const API = [
  { group: "Mare", of: "mare" as const, calls: [
  ...
  { group: "Vongola", of: "vongola" as const, calls: [
  ...
  { group: "Arcobaleno", of: "arcobaleno" as const, calls: [
          ...
          {API.map((g) => (
            <div key={g.group} className="bg-panel">
              <div className="flex items-center gap-2.5 border-b border-line px-4 py-3">
                ...
                <span className="font-mono text-[11px] tracking-[0.16em] text-fg-2">{g.group.toUpperCase()}</span>
              </div>
              <div className="p-1">
                {g.calls.map(([sig, note]) => (
                  <div key={sig} className="px-3 py-2.5">
                    <code className="block font-mono text-[11.5px] text-fg">{sig}</code>
                    <span className="mt-1 block text-[11px] text-fg-3">{note}</span>
                  </div>
                ))}
              </div>
            </div>
...
`}<span className="text-ok">{`assert s_a2.id === s_a.id   ✔`}</span>
          ...
          <div className="bg-panel">
            ...
            <div>
              ...
              <Row k="Schema invariant tests" v="24" note="17 rejections" tone="ok" />
              <Row k="Worlds in the worked grid" v="16" note="1 primary" />
              <Row k="Irreversible actions suppressed" v="15" note="by trigger" tone="accent" />
        ...
        <p className="mt-6 max-w-[78ch] text-b1 text-fg-3">
          That last pair is a correction the implementation forced. Folding provenance into a single
          node hash made cross-world sharing impossible by construction — the grid stored 46 objects
          for 21 distinct facts. Identity is now split from payload, so provenance stays
          hash-sensitive while storage does not.
        </p>
      </Section>
          ...
          <Panel className="transition-colors group-hover:border-fg-4">
            <div className="grid md:grid-cols-[1.15fr_1fr]">
              <div className="p-6 md:p-8">
                ...
                <h3 className="mt-3 text-h2 text-fg">Airport queue intelligence</h3>
                <p className="mt-4 max-w-[48ch] text-b2 text-fg-2">
                  The traveller&apos;s real question is not <em className="not-italic text-fg-3">how busy is the
              ...
              <div className="border-t border-line md:border-t-0 md:border-l">
                {[
                  ["mare", "Mare", "Departure-time scenarios — mutually exclusive"],
                  ["vongola", "Vongola", "This terminal's own queue history"],
                  ["arcobaleno", "Arcobaleno", "The checkpoint state right now"],
                ].map(([k, n, d]) => (
                  <div key={n} className="flex items-start gap-3 border-b border-line-soft px-6 py-4 last:border-b-0">
                    <span className="mt-[1px]"><Axis of={k as "mare"} size={17} /></span>
                    <div>
                      <div className="font-mono text-[11px] tracking-[0.14em] text-fg-2">{n.toUpperCase()}</div>
