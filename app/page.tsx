...
const TRIAD = [
  {
    ...
    calls: [
      ...
      ["graft(source, selector, mode)", "typed transfer, observed | assimilated"],
...
const INVARIANTS: [string, string, string, string, string?][] = [
  ["I1", "Snapshots are immutable and content-addressed",
   ...
   "/examples/airport"],
  ["I2", "Worlds are append-only; advance never rewrites",
  ...
  ["I3", "fork(W, s) cannot affect W",
   ...
   "/examples/airport"],
  ["I4", "materialize(sid) is deterministic and idempotent",
   ...
   "/examples/airport"],
  ["I5", "Every object carries provenance to an origin",
   ...
   "/examples/airport"],
  ["I6", "Irreversible effects only from the primary world",
   ...
   "/examples"],
  ["I7", "The ontology mutates only via a declared Action",
   ...
   "/examples/oil"],
  ["I8", "Every cross-world transfer is a lateral edge",
    ...
    <>
      ...
      <header className="sticky top-0 z-20 border-b border-line bg-bg/85 backdrop-blur">
        <div className="mx-auto flex max-w-[1180px] items-center gap-8 px-6 py-3.5 md:px-10">
          ...
          <nav className="ml-auto flex items-center gap-6">
            ...
            <Link href="/examples/airport" className="text-[12.5px] text-accent transition-colors hover:text-fg">Try it</Link>
          </nav>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1180px] px-6 pt-8 pb-14 md:px-10 md:pt-12 md:pb-20">
        ...
        <p className="mt-8 max-w-[54ch] text-b4 text-fg-2">
          Trinisette is a branching state substrate that lets AI agents execute, reproduce, and
          transfer experience across parallel worlds and time. Fork a world, act inside it, and
          replay it later to the byte.
        </p>

        <div className="mt-10 flex flex-wrap items-center gap-3">
          <Link href="/cases/airport"
        ...
        */}
              ...
              <div className="mt-5 border-t border-line-soft pt-4">
                ...
                <div className="mt-2 space-y-2.5">
                  {t.calls.map(([sig, note]) => (
                    <div key={sig}>
                      <code className="block font-mono text-[11.5px] text-fg">{sig}</code>
                      <span className="mt-0.5 block text-[11px] leading-snug text-fg-3">{note}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <p className="mt-5 max-w-[86ch] text-b1 text-fg-3">
          One primitive group per axis, and there is no generic write. Every step is an invocation
          of an action somebody declared in advance, with typed parameters, preconditions and a
          fixed effect class.
        </p>
        ...
        <Panel className="mt-5 p-6">
          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
            ...
            <span className="text-b2 text-fg">Experience is not a fourth primitive.</span>
          </div>
          <p className="mt-3 max-w-[78ch] text-b1 text-fg-2">
            It has no axis of its own — it is inheritance pointed sideways. Vongola is the vertical
            edge, from a snapshot to its predecessor in the same world; Experience is the horizontal
            edge, from a snapshot to one in another world. A grafted snapshot therefore has two
            parents, which makes the snapshot graph a <span className="text-fg">DAG, not a tree</span>,
      ...
      {/* ── 04 invariants ───────────────────────────────────────────── */}
      <Section
        id="invariants" index="02" title="Invariants, and how to check them"
          ...
          {INVARIANTS.map(([id, rule, why, check, href]) => (
            <div key={id}
              ...
              <span className="text-[13px] leading-relaxed text-fg-2">
                {check}{" "}
                {href && (
                  <Link href={href} className="whitespace-nowrap font-mono text-[11.5px] text-accent hover:text-fg">
                    open →
                  </Link>
                )}
              </span>
        ...
        <div className="mt-px grid gap-px border border-line bg-line md:grid-cols-4">
          {[
            ...
            <div key={n} className="bg-panel px-4 py-4">
              ...
              <div className="mt-1.5 text-[12px] leading-snug text-fg-3">{d}</div>
            </div>
          ))}
        </div>

        <p className="mt-6 max-w-[86ch] text-b1 text-fg-3">
          The site runs an in-memory port of these rules so the console works in a browser. The
          version that enforces them the way the argument requires — as PostgreSQL constraints and
          triggers, where a violation is rejected by the database rather than by a code path someone
          remembered to write — lives in{" "}
          <code className="font-mono text-[13px] text-fg-2">reference/</code> and is excluded from
          this build. Two of the eight were only stated correctly after that engine rejected the
      ...
      {/* ── 04 domains ─────────────────────────────────────────────────── */}
      <Section
        index="03" title="Four domains, one substrate"
      ...
      >
          ...
          {EXAMPLES.map((e) => (
            ...
            >
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                ...
                <code className="font-mono text-[11.5px] text-fg-4">/examples/{e.slug}</code>
                <span className={`ml-auto font-mono text-[9.5px] uppercase tracking-[0.14em] ${
                  e.status === "live" ? "text-ok" : "text-warn"
                }`}>{e.status}</span>
              </div>

              <p className="mt-3 max-w-[46ch] grow text-b1 text-fg-2">{e.blurb}</p>
        ...
        <p className="mt-6 max-w-[86ch] text-b1 text-fg-3">
          ...
          <span className="text-fg-2">irreversible action</span> is a push notification, a binding
          contract, a public warning and a berth commitment — four different weights, each declared
          per action rather than assumed per application. The{" "}
          <span className="text-fg-2">coverage gap</span> is a missing queue feed, an unpriced grade,
          a sparse station network and a terminal with no arrivals — four different holes, all
          answered by the same lateral edge. None of them needed the engine changed.
        </p>

        <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-2">
          <Link href="/cases/airport" className="font-mono text-[12px] text-accent hover:text-fg">
      ...
      <footer className="border-t border-line">
        <div className="mx-auto flex max-w-[1180px] flex-wrap items-center gap-x-8 gap-y-3 px-6 py-8 md:px-10">
          <Triad />
          <Label className="text-fg-4">rev. 3 · phases 0–3 implemented</Label>
          <span className="ml-auto font-mono text-[10.5px] text-fg-4">
            Mare divides worlds · Vongola carries experience through time · Arcobaleno is the state at that point
          </span>
        </div>
      </footer>
    </>
